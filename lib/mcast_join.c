...
int mcast_join(int sockfd, const SA *grp, socklen_t grplen, const char *ifname, u_int ifindex)
{
	...
	if(ifindex > 0) {
		...
		if((req.gr_interface = if_nametoindex(ifname)) == 0) {
			errno = ENXIO;					// i/f name not fount
			return -1;
		}
	} else {
		req.gr_interface = 0;
...
#else
	switch(grp->sa_family_t) {
	case AF_INET: {
		struct ip_mreq mreq;
		struct ifreq ifreq;

		memcpy(&mreq.imr_multiaddr, &((const struct sockaddr_in *) grp)->sin_addr, sizeof(strcut in_addr));
		...
		if(ifindex > 0) {
			...
			strncpy(ifreq.ifr_name, ifname, INFNAMSIZ);
doioctl:
