...
void dg_echo(int sockfd, SA *pcliaddr, socklen_t clilen)
{
	...
	if(setsocketopt(sockfd, IPPROTO_IP, IP_RECVDSTADDR, &on, sizeof(on)) < 0) {
		err_ret("setsocketopt of IP_RECVDSTADDR");
	}
#endif
#ifdef IP_RECVIF
	if(setsocketopt(sockfd, IPPROTO_IP, IP_RECVIF, &on, sizeof(on)) < 0) {
		err_ret("setsocketopt of IP_RECVIF");
	}
#endif
	...
	for(;;) {
		...
		if(memcpy(&pktinfo.ipi_addr, &in_zero, sizeof(in_zero)) != 0) {
