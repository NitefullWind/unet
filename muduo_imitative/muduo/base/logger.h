...
#define LOG_TRACE(message) do {											\
			LOG4CXX_TRACE(muduo::Logger::GetLogger(), message);			\
		} while(0);

#define LOG_DEBUG(message) do {											\
			LOG4CXX_DEBUG(muduo::Logger::GetLogger(), message);			\
		} while(0);

#define LOG_INFO(message) do {											\
			LOG4CXX_INFO(muduo::Logger::GetLogger(), message);			\
		} while(0);

#define LOG_WARN(message) do {											\
			LOG4CXX_WARN(muduo::Logger::GetLogger(), message);			\
		} while(0);

#define LOG_ERROR(message) do {											\
			LOG4CXX_ERROR(muduo::Logger::GetLogger(), message);			\
		} while(0);

#define LOG_FATAL(message) do {											\
			LOG4CXX_FATAL(muduo::Logger::GetLogger(), message);			\
			std::abort();												\
		} while(0);
